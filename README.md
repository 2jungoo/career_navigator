...
| SBERT Hybrid + Threshold | 90.5% | [84.5, 96.4] | 90.7% | 91.7% | — |
| **SBERT + Learned Head (nested LOOCV)** | **79.8%** | [71.4, 88.1] | **0.786** | 100% | leakage 없는 정직한 수치 |
...
> 합성셋 97% → 실데이터 72%. 실제 자소서의 도메인 혼재·표현 다양성에 따른 일반화 갭을 정직하게 공개한다. 단 실데이터에서는 Macro F1 기준 SBERT Hybrid(86.4%)가 Keyword(79.0%)를 더 뚜렷이 앞서, 의미 임베딩의 가치가 합성셋보다 분명해진다. (수치 재현: `node scripts/runFullEvaluation.mjs`)
